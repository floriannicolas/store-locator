...
function StoreSchedules({ store }: { store: Store }) {
  ...
  const currentOpeningHoursLabel = useMemo(() => {
    const currentDay = dayjs().format('dddd').toLocaleLowerCase() as keyof typeof store.openingHours
    ...
    return (
      <>
        <span className="text-red-600 font-medium">Fermé</span> - Ouvre à {currentOpening.openTime}
      </>
    )
  }, [store])
    ...
    return (
      <ul className="w-full max-w-sm">
        {sortedDays.map(([day, frenchDay]) => {
          const opening = store.openingHours[day as keyof typeof store.openingHours]
